...
describe("createNavigationSync", () => {
  ...
  describe("run", () => {
    it("bypasses throttle and awaits pull then push", async () => {
      const deps = createDeps();
      const callOrder: string[] = [];
      deps.pullSync.mockImplementation(async () => {
        callOrder.push("pull");
      ...
      // Within throttle window, run() still executes and awaits completion
      vi.advanceTimersByTime(1000);
      await sync.run();
      expect(callOrder).toEqual(["pull", "push", "pull", "push"]);
    });

    it("skips when offline", async () => {
      const deps = createDeps();
      deps.isOnline.mockReturnValue(false);
      await createNavigationSync(deps).run();
      expect(deps.pullSync).not.toHaveBeenCalled();
      expect(deps.syncAll).not.toHaveBeenCalled();
    });

    it("throttles a following trigger", async () => {
      const deps = createDeps();
      const sync = createNavigationSync(deps);
      await sync.run();
      expect(deps.pullSync).toHaveBeenCalledTimes(1);
    ...
    it("waits for a busy mutex before pulling", async () => {
      const mutex = createSyncMutex();
      const deps = createDeps(mutex);
      let resolveExternal: () => void;
      const externalBlocks = new Promise<void>((r) => {
    ...
    it("gives up waiting after the cap and still pushes", async () => {
      const mutex = createSyncMutex();
      const deps = createDeps(mutex);
      const externalPromise = mutex.run(() => new Promise<void>(() => {}));
      void externalPromise;
