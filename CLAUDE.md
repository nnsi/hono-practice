...
### テストの削除・スキップの判断
- **同期関連機能のテストは削除またはスキップすること**
  - オフライン同期機能は削除されたため、関連するテストは不要
  - `it.skip()` を使用して、削除理由をコメントに残す
  
### テスト修正時の重要事項
- **型エラーを全て修正してテストも全て通らないとコミットできない**
  - `npm run test-once` で全てのテストが通ることを確認
  - `npm run tsc` でTypeScriptのコンパイルエラーがないことを確認
  - `npm run fix` でコードフォーマットを整える
  
- **アプリケーションが正常動作していてもテストは必ず全て通るようにする**
  - テストの期待値が古い場合は、現在の実装に合わせて更新する
  - APIパラメータフォーマットの変更（例：mutation のパラメータ構造）に注意