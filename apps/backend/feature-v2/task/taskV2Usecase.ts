...
export function newTaskV2Usecase(repo: TaskV2Repository): TaskV2Usecase {
  return {
    getTasks: getTasks(repo),
    syncTasks: syncTasks(repo),
  };
}

function getTasks(repo: TaskV2Repository) {
  return async (
    userId: UserId,
    since?: string,
  ): Promise<{ tasks: TaskRow[] }> => {
    const result = await repo.getTasksByUserId(userId, since);
    return { tasks: result };
  };
}

function syncTasks(repo: TaskV2Repository) {
  return async (
    userId: UserId,
    taskList: UpsertTaskRequest[],
  ): Promise<SyncResult> => {
    ...
    const upserted = await repo.upsertTasks(userId, validTasks);
    ...
    if (missedIds.length > 0) {
      serverWins = await repo.getTasksByIds(userId, missedIds);
