...
2. `.env` を export した状態で `eas env:list` を実行し、配信環境の `EXPO_PUBLIC_*` を確認する。

```bash
bash -lc "cd apps/mobile && set -a && source .env && set +a && eas env:list --environment preview"
```

3. `docs/ops/mobile-ota.md` に従って `EXPO_PUBLIC_*` を付けたまま Android と iOS を個別に配信する。
- Android は `--clear-cache` を付ける
- `--platform all` は使わない

4. Update ID と Dashboard URL を報告する。
