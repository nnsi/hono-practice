...
- この skill が発火したら、実装前に必ず `update_plan` で Phase を並べる
- 実装に入る前に、Phase 1 から 5 を `completed` にする
- 不要な Phase は、不要理由を共有したうえで `completed` 扱いにして先へ進む
- Phase 5 は default で `Worktree Setup` として扱い、Codex の裁量で現在の workspace 利用へ切り替えない
- 後続 Phase の作業を始める前に、直前までの Phase が完了していることを確認する
- `apply_patch`、ファイル作成、dev server 起動、`pnpm run test-once`、`pnpm run tsc`、ブラウザ確認、コミットは、対応する直前 Phase を飛ばして始めてはいけない
- 後続 Phase を先に始めてしまったことに気づいたら、そのまま続行せず、最も手前の未完了 Phase に戻る
...
- `apps/mobile` が影響範囲に入る場合は、`OTA で済む変更か` `native build が必要になる変更か` を確認対象として最初から立てる
...
- `apps/mobile/package.json` や Expo / native 設定に触る可能性がある場合は、`OTA 可否` と `再 build 必要有無` をユーザー判断が必要な論点として必ず確認候補に入れる
...
- モバイル案では `JS-only / OTA 可能` と `native 依存追加 / 再 build 必須` を明示的に分類する
- native 依存追加案を採る場合は、実装前に `導入 package`, `再 build 必要`, `配布や課金影響` をユーザーへ共有して承認を得る
- おすすめ案と理由を示し、ユーザーが未承認なら選択を求める
- ユーザーが事前に `承認不要` `おすすめで進めてよい` と明示している場合だけ、おすすめ案で進める

### 5. Worktree Setup

- `feature-dev` では `.codex/skills/worktree-setup/SKILL.md` を使って worktree を作る
- PowerShell セッションでも setup / cleanup script は `bash ./scripts/...` 形式で実行する
- 既存 worktree のパスやブランチがユーザーから指定されている場合はそれを使う
- ユーザーが明示的に `worktree 不要` `main で作業` などと指示した場合だけ skip する
- `確認不要` `任せる` `そのまま進めてよい` は skip 条件ではない
- worktree パス、ポート、DB 名、使用ブランチを plan に残す

### 6. Implementation + Verification

- Phase 1 から 5 の完了が確認できていない限り開始しない
- 実装は worker に委譲してよいが、ファイル責務を分離する
- 新規ルートを足すフロントエンド変更では dev server を先に起動する
- `apps/mobile/package.json` や Expo / native 設定を編集する前に、Phase 3 と 4 で native 影響と承認が処理済みかを再確認する
...
- cleanup script も `bash ./scripts/worktree-cleanup.sh <name>` で実行する
- 開発サーバーはポート指定で止める

## 注意事項

- Phase を飛ばさず、`update_plan` を更新し続ける
- 不要として閉じた Phase は、どのユーザー発話またはローカル事実が根拠なのかを短く明示してから先へ進む
