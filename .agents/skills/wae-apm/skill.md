...
- **WAEデータセット名**:
  - `actiko_api_logs` — APIログ（サーバーサイド）
  - `actiko_client_errors` — クライアントエラー（フロントエンド）
...
- **`component_error`**: React ErrorBoundaryで捕捉。コンポーネントスタック付き
- **`unhandled_error`**: `window.onerror` または `unhandledrejection` で捕捉
- **`network_error`**: スキーマに定義済みだが現状未使用（sync失敗はサイレントリトライのみ）
- **スタックが空 or 無意味**: minified buildのため。Source map連携がないと詳細追跡は困難
- **`screen` が空**: getContext未設定の古いビルドからの送信
- **GIS関連エラー**: アドブロッカーやネットワーク不安定で頻発する既知パターン。深刻度は低い
