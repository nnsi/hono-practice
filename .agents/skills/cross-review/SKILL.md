...
Codex subagent (Logic+Security) と Codex (Architecture+Testability) の2体による軽量レビュー。**修正は行わない。**
...
| # | エンジン | 専門領域 |
|---|---------|----------|
| A | Codex subagent (`reviewer-logic`, Opus) | ロジック・バグ + セキュリティ |
...
#### A: Codex subagent (Logic + Security)
...
- `--write` は付けない（読み取り専用）
- `--background` は付けない（Codex側の `run_in_background: true` で並列化する）
...
### 各レビュアー判定
- A (Logic+Security / Codex): LGTM / NOT LGTM
