# Codex: Swift/Kotlinネイティブコードレビュー

Codex CLIに渡すプロンプトテンプレート。レビュー対象にネイティブコードが含まれる場合のみ使用。

## プロンプト

```
以下のSwift/Kotlinファイルをレビューしてください。TypeScriptファイルはレビュー対象外です。
...
## レビュー観点
1. **コンパイル安全性**: 型エラー、protocol/interface準拠漏れ、API level制約、import不足
2. **iOS/Android対称性**: 片方だけ修正してもう片方が漏れていないか、振る舞いの乖離
