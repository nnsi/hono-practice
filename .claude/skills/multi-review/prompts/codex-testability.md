...
- バックエンド層: route → handler → usecase → repository

## レビュー観点
1. **テストカバレッジ**: 新ロジックにテストがあるか、分岐カバレッジは十分か
2. **テストケース網羅性**: 正常系・異常系・境界値・非同期（成功/失敗/タイムアウト）
3. **テスト品質**: 実装依存度、モックの適切さ、AAA パターン、テスト独立性
4. **テスタビリティ**: テストしにくい構造がないか、依存注入(newXXX)の活用、副作用分離
5. **Hono固有**: executionCtx依存の処理、noopTracerフォールバック、fireAndForget
