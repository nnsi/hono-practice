...
- サブエージェントにはChrome MCPのアクセス権がない。Chrome MCPでのブラウザ確認は並列化できない
- **サブエージェントでの並列ブラウザ確認が必要な場合は `playwright-cli` を使う**。`-s <session名>` で複数セッションを同時に起動できる。詳細は `/playwright-cli` スキル参照
