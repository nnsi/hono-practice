...
#### Web/Mobile ロジック共通化の推進

現在 `frontend-v2` と `mobile-v2` で同一ロジックが重複している箇所（stats計算、目標ライン生成等）を `packages/` の共通パッケージに純粋関数として切り出す。コロケーション型Hooks（`use*.ts`）のうちデータ変換・ビジネスロジック部分が対象。データ取得層はプラットフォーム固有のまま残す。

前提: mobile-v2のsnake_case→camelCase変換をrepository層で完結させる。

メリット:
- 1箇所の修正でWeb/Mobile両方に反映される（エージェント運用で特に効果大）
- 共通ロジックに対してテストを1セット書けば両プラットフォームをカバー

将来的にUI層の共通化（コンポーネントインターフェースの統一等）も検討しうるが、デザイン投資フェーズとのトレードオフがあるため、ロジック共通化の効果を見てから判断する。
