...
| 場所 | 内容 | 問題 |
|------|------|------|
| 旧 backend domain ディレクトリ | Zodエンティティ定義（discriminated union）、branded ID | バックエンド専用。フロントエンドから参照できない |
| `apps/frontend/src/hooks/` + `components/` | Dexie フィルタリング・ソート・stats計算ロジック | フロント専用。バックエンドのロジックと二重管理 |
...
- 旧 backend domain ディレクトリにはバックエンド専用ドメイン（auth, subscription, apiKey）も存在する
...
旧 backend domain ディレクトリは廃止し、全ドメインを `packages/domain/` に集約する。
...
```typescript
// 例: packages/domain/task/taskSchema.ts
...
- 実装計画は当時の作業メモとして運用し、現状はコミット履歴と ADR 群を参照する
