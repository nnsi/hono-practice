...
影響箇所:
- 旧 frontend-v2 ディレクトリ — Webフロントエンド
- 旧 mobile-v2 ディレクトリ — モバイルアプリ
- 旧 feature-v2 ディレクトリ — 同期API群（activity, activity-log, goal, goal-freeze-period, task）
- feature-v2 配下の全ファイル名（`activityV2Route.ts` 等、V2サフィックス付き）
- ルートpackage.jsonのスクリプト（`actiko-frontend-v2` フィルタ）
- 旧 frontend package 名 `actiko-frontend-v2`
- CLAUDE.md、skills、旧 knowledge docs などのドキュメント参照

## 決定事項

### 1. 旧 frontend-v2 → `apps/frontend/`
- package.json name: `actiko-frontend-v2` → `actiko-frontend`
- ルートpackage.jsonのpnpmフィルタを更新

### 2. 旧 mobile-v2 → `apps/mobile/`
- package.json nameは既に `actiko-mobile` で変更不要

### 3. 旧 feature-v2 → `apps/backend/feature-sync/`
