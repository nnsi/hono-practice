...
破棄（2026-09-18。`20260918_close_mobile_dispatch.md` で置換）
...
モバイルのリリース操作は GitHub Actions `deploy.yml` の `workflow_dispatch` で、環境（stg / prod）と操作（build / OTA）を手動選択して実行する。  
通常の PR 品質ゲートは `pr.yml` に集約し、mobile の typecheck / test / Expo Web export を含める。
...
- CI/CD は `pr.yml` と `deploy.yml` の 2 workflow 構成に整理
- mobile リリースは `deploy.yml` の `workflow_dispatch` から手動実行
- PR では mobile の typecheck / test / Expo Web export を実行
- EAS の無料枠（月 30 ビルド / 月 1,000 OTA）の範囲で十分に運用可能
- チーム規模の拡大やリリース頻度の増加時に、専用 workflow 分離や自動検知の追加を再検討する
