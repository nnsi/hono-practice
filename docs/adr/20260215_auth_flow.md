...
1. `@packages/auth-client`の`createAuthenticatedFetch()`がアクセストークンを自動付与
...
**サーバー側にgrace period（replay window）を 30 秒設ける + クライアント側のmutexで多重リクエストを防止する。**

> ⚠ 2026-05-14 更新: 当初は「サーバー側 grace なし」方針だったが、lost-response race
> (rotation 成功直後にレスポンスが消失 → クライアントが旧 token を retain) で強制ログアウト
> される事例が発生。「1 旧 token → 最大 1 救済」の不変条件付きで 30 秒 grace を採用した。
> 詳細は ADR `20260514_refresh_token_state_machine.md` を参照。

理由:
- クライアント側 mutex のみでは「タブ間 race」「lost-response」を全部は塞げない
- grace 救済は CAS UPDATE + `rotatedAt` フラグで「1 旧 token = 1 救済」に制限
- 多重リクエストの発生源（タブ復帰、ネットワークリトライ等）はクライアント側 mutex で先に抑える
...
- 共通: `packages/auth-client/http/createAuthenticatedFetch.ts` (Web/Mobile 両用、refresh は注入型)
- Web の refresh 実装: `apps/frontend/src/auth/webAuthTransport.ts` (httpOnly cookie 経由)
- Mobile の refresh 実装: `apps/mobile/src/auth/mobileAuthTransport.ts` (SecureStore + Bearer)
