...
> 調査日: 2026-03-08
> 調査対象: frontend-v2 の「auth後回し・即UI表示」改修に伴うリスク分析（mobile-v2にも同等修正を適用）
...
#### H1: `upsertFromServer` の `bulkPut` が pending レコードを無条件上書き ✅ 修正済み (2026-03-08) [frontend-v2 / mobile-v2]
...
#### H2: 別アカウントのキャッシュ露出 + データ漏洩 ✅ 修正済み (2026-03-08) [frontend-v2 / mobile-v2]
...
#### H3: userId空文字 or 旧ユーザーIDでの書き込み ✅ 修正済み (2026-03-08) [frontend-v2 / mobile-v2]
...
#### H4: ログアウト/アカウント切替時に飛行中のsyncがDexieに書き戻す ✅ 修正済み (2026-03-08) [frontend-v2 / mobile-v2]
...
#### H5: push失敗がsyncEngine上は成功扱い ✅ 修正済み (2026-03-08) [frontend-v2 / mobile-v2]
...
#### H6: オフライン起動時にsyncReadyが永久にfalseのまま ✅ 修正済み (2026-03-08) [frontend-v2 / mobile-v2]
...
**mobile-v2の修正**: frontend-v2と同じパターン。`authGenRef`（世代番号）による飛行中キャンセル＋`onlineRetryRef`（`NetInfo.addEventListener`）によるネットワーク復帰時リトライ。ログアウト時に世代番号インクリメント＋リスナー解除。

---

#### H7: React Queryキャッシュがログアウト時に未クリア ✅ 修正済み (2026-03-08) [frontend-v2のみ]
...
**mobile-v2**: N/A（TanStack Queryを使用していない）

---

### MEDIUM: 体験劣化。許容判断が必要

#### M1: initialSyncがテーブルごとに個別書き込み → 中間状態がuseLiveQueryに露出 ✅ 修正済み (2026-03-08)
...
**修正内容**: APIレスポンスのパースとDB書き込みを分離し、全テーブル（activities, activityKinds, activityLogs, goals, tasks）への書き込みをトランザクションでラップ。frontend-v2は `db.transaction()`（Dexie）、mobile-v2は `db.withTransactionAsync()`（expo-sqlite）。コミット後にリアクティブクエリが1回だけ発火する。
...
**影響**: 数値がsync完了時にジャンプする（例: 昨日のログが反映されていない状態で目標達成率が低く見える）。`useLiveQuery` がsync完了後に自動再発火するため値は最終的に正しくなる。
...
- **発見元**: Codex
- **判断**: 後回し（軽微。バックエンドが冪等なので実害はAPIコスト増のみ）
...
**対処案（将来）**: `BroadcastChannel` API or Dexie の `Dexie.on('changes')` でリーダー選出。またはAPI側でPOST bodyのハッシュによる短期間キャッシュ（重複リクエストを無視）。

---

#### M4: チャンク途中の失敗でサーバー適用済み + ローカルpendingの乖離 ✅ 修正済み (2026-03-08)

- **発見元**: Codex

**問題**: syncのチャンクループで `chunk[0]` 成功 → `chunk[1]` 失敗 → `return` で全件のmark処理がスキップされる。`chunk[0]` 分はサーバーに反映済みだがローカルは `_syncStatus: "pending"` のまま。

**対象コード**: `syncActivities.ts`, `syncActivityLogs.ts`, `syncGoals.ts`, `syncTasks.ts`

**緩和要因**: 次回syncで再送されるが、バックエンドは冪等なので実害なし。ただしsyncEngine上は成功扱い（H5と関連）。

**修正内容**: 全チャンク完了後の一括mark（`mergeSyncResults`）から、チャンクごとの即座mark処理に変更。各チャンクのAPI成功直後に `markSynced` / `markFailed` / `upsertFromServer` を実行する。途中チャンクが失敗しても、それ以前に成功したチャンクのmark結果はDB上で確定している。frontend-v2・mobile-v2の両方に適用。
...
| 優先 | ID | 内容 | 既存バグ？ | 改修で悪化？ | ステータス | mobile-v2 |
|------|----|------|-----------|------------|-----------|-----------|
| 1 | H1 | bulkPutのpending上書き | Yes | ウィンドウ拡大 | ✅ 修正済み | ✅ 修正済み |
| 2 | H5 | push失敗が成功扱い | Yes | 変化なし | ✅ 修正済み | ✅ 修正済み |
| 3 | H2 | 別アカウントデータ漏洩 | Yes（1h以内） | ウィンドウ拡大 | ✅ 修正済み | ✅ 修正済み |
| 4 | H4 | 飛行中syncの書き戻し | Yes | 変化なし | ✅ 修正済み | ✅ 修正済み |
| 5 | H3 | 空userId書き込み | No | 新規発生 | ✅ 修正済み | ✅ 修正済み |
| 6 | H6 | syncReady永久false | No | 大幅悪化 | ✅ 修正済み | ✅ 修正済み |
| 7 | H7 | React Queryキャッシュ漏洩 | Yes | 変化なし | ✅ 修正済み | N/A |
| 8 | M1 | initialSync中間状態露出 | 部分的 | 頻度増加 | ✅ 修正済み | ✅ 修正済み |
