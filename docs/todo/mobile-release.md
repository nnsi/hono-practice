...
- [x] EAS が自動管理: `eas credentials` で Provisioning Profile & Certificate を生成済み

### Android

- [x] Upload Keystore: 初回 production ビルド時に EAS が自動生成
- [x] Google Play App Signing: 初回 AAB アップロード時に自動有効化
...
- [x] Google Cloud Console でサービスアカウントキーを作成し、Google Play Console の [ユーザーと権限] からリリース管理権限を付与
...
- [ ] Google Play Console にアップロード:
  ...
  # 初回実行時に対話形式で Google Service Account Key の登録を求められる
  # 「Upload a Google Service Account Key」を選択 → google-service-account.json のパスを入力
  # 2回目以降は EAS に登録済みのキーが自動で使われる
  ```
- [ ] **内部テスト**（審査なし・即配布）
...
> Section 8 でビルド・ベータテスト済みの同じバイナリをそのまま使う。再ビルド不要。

### iOS

- [ ] App Store Connect でビルドを確認（TestFlight に上がっているもの）
- [ ] 審査に提出
- [ ] 審査フィードバック対応（リジェクト時）
- [ ] 承認後リリース（手動 or 自動）
