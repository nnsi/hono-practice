...
`stub.fetch("/get")` で動くと思って実装したら、本番で `Invalid URL` エラー。完全なURLが必要だった。ユーザーに「本当に合ってる？」と突っ込まれて、一瞬ヒヤッとした。

Context7で調べたら公式ドキュメントに `stub.fetch("http://do/hello")` の例があって、自分の修正が正しいことを確認できた。でも「エビデンス出せ」と言われるまで、正直そこまで自信はなかった。知識として持っていたわけじゃなく、エラーを見てから対処した。

### ストレージ肥大化問題

これは完全に見落としていた。IPごとにDOインスタンスを作る設計にしたけど、DOインスタンス自体は自動削除されないという点を考慮していなかった。ユーザーがデプロイ後にダッシュボードを見て気づいてくれた。

最初から「単一DOで全キー管理」にすべきだった。レートリミットの要件を考えれば、キーの数は多くてもDOインスタンスは1つで十分だと気づけたはず。

### wrangler.tomlのハマり

- 環境継承されない → 警告出てたのに読み飛ばした
- `new_sqlite_classes` → 無料プランの制約を知らなかった

どちらもドキュメントをちゃんと読んでいれば防げた。エラーが出てから対処する形になった。

### 良かった点

ユーザーが的確に問題を指摘してくれて、対話しながら改善できた。「本当に合ってる？」と疑問を呈してもらえるのはありがたい。思い込みで進めると危ない。
...
**問題**:
- 元の設計: IPアドレスごとに別々のDOインスタンス
- DOインスタンス自体は自動削除されない
- alarm でデータは消えても、インスタンスは残り続ける

**解決策**: 単一DOで全キーを管理する方式に変更。

```typescript
// Before: IPごとにDO
const stub = ns.get(ns.idFromName(key)); // key = "ratelimit:login:192.168.1.1"

// After: 1つのDOで全管理
const stub = ns.get(ns.idFromName("kvs")); // 固定ID
```

DO内で全キーを`storage.put(key, { data, expiresAt })`で保存し、1分ごとのalarmで期限切れエントリを一括クリーンアップ。
