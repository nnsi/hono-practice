...
デプロイ後にCloudflareのダッシュボード見てたら、DOのSQLiteストレージがじわじわ増えてることに気づいた。30リクエストで32KB程度だけど、IPごとにDOインスタンスが作られる設計だったので、このまま放置するとストレージが圧迫される。
...
**学び**:
- DOは「インスタンス数」も意識する必要がある
- 最初からこの設計にしておけばよかった
- でも、動かしてみて初めて気づく問題もある。本番で気づけてよかった

実装してすぐ問題に気づけたのは良かった。「動いたからOK」じゃなくて、リソース消費まで見る癖をつけたい。
