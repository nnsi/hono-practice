...
## mobile-v2 Hono RPCクライアント移行

前のセッションの残件だった「apiClient Hono RPC移行」をやった。

### 調査フェーズ

最初にContext7でHonoのRPCドキュメントを読み、GitHubのDiscussion（#3642, #3339）も確認した。React NativeでHono RPCを使う場合の既知問題は1つ：MetroバンドラーがHonoのpackage.jsonのexports mapを解決できない。ワークアラウンドは`metro.config.js`で`hono/client`を`hono/dist/client/index.js`に直接マップすること。

frontend-v2が既にHono RPC（`hc<AppType>`）を使っていたので、そのパターンを踏襲する方針は即決だった。モノレポ構成なので`AppType`の共有も問題なし。

### 実装

変更は6ファイル＋設定ファイル3つ。作業自体は30分程度。

- `apiClient.ts`: `hc<AppType>(API_URL, { fetch: customFetch })`を追加。sync/fetch系の手動wrapper関数（`apiSyncActivities`等10個）を削除。認証系（`apiLogin`, `apiRegister`等）はトークン管理が絡むので手動のまま残した
- sync 4ファイル: `apiClient.users.v2.activities.sync.$post({ json: ... })`のようなRPC呼び出しに書き換え
- `initialSync.ts`: 同様にRPC化
- アイコン操作（upload/delete）はバックエンドがzValidatorを使っていないためRPCの型推論が効かず、`customFetch`直呼びを維持。これはfrontend-v2も同じアプローチ

### honoバージョン地獄

一番時間がかかったのは実装ではなくpnpmの依存解決。最初に`pnpm add hono`したら4.12.3が入り、backendは4.12.1。同じ`Hono`型でもパッケージインスタンスが違うので`hc<AppType>`が`AppType does not satisfy the constraint 'Hono<any, any, any>'`で型エラー。

`pnpm add hono@4.12.1`で揃えたつもりが、`^4.12.1`の指定でlockfileは4.12.1を指しているのに、root `package.json`の`@hono/node-server`がpeer dependencyで4.12.3を引っ張っていて、root `node_modules/hono`が4.12.3のまま。`packages/frontend-shared`のtscがこれを拾って型エラー7件。

結局、root `package.json`に`pnpm.overrides`で`"hono": "4.12.1"`を追加して全面統一した。

今日の日記の上にも「pnpmのphantom dependency問題は前にもBottomTabBarPropsで遭遇していた」「monorepoでの依存管理は本当に地雷が多い」と書いたが、同じ日に同じ種類の問題にまた引っかかっている。drizzle-ormの488件エラーもpnpmの重複インストールだったし。このプロジェクトのpnpm依存管理はそろそろ体系的に整理した方がいい。

### 動作確認

ブラウザ（Chrome MCP）で確認。ネットワークリクエストを見ると：

- 初期同期: `GET /users/v2/activities`, `/activity-logs`, `/goals`, `/tasks` → 全て200
- アクティビティ記録: `POST /users/v2/activity-logs/sync` → 200
- 各タブ（Daily, Goal）のレンダリングも正常

### Hermesの懸念は杞憂だった

前のセッションの日記で「`hc`は内部で`URL`オブジェクトを使うが、Hermes（RNのJSエンジン）のURL実装が完全かどうかは少し心配」と書いた。結果的にWeb環境では問題なく動いた。ただし、これはExpo Web（ブラウザ上）での確認なので、実機（Hermes）での動作はまだ未検証。実機テストはユーザーに任せるしかないが、Discussion #3642のワークアラウンド（metro.config.jsの解決ルール）がネイティブでも効くはずなので、大丈夫だろうと思っている。

### 残件の更新

- ✅ apiClient Hono RPC移行 ← 今回完了
- ⏳ @packages/frontend-shared（APIキー/サブスクリプション）
- ⏳ ローカルファイル共通化
